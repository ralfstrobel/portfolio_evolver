...
use self::serde_json::{
    ser::PrettyFormatter as JsonPrettyFormatter, Map as JsonObject, Serializer as JsonSerializer,
    Value as JsonValue,
};
...
impl ChartData {
    ...
    pub fn load(name: &String, from_time: i64, to_time: i64) -> Self {
        ...
        if (from_time > 0) && (start_timestamp > from_time) {
            panic!(
                "Data set {} does not begin until after requested start date.",
                name
            );
        }
    ...
    fn assert_aligned_with(&self, other: &Self) {
        ...
        {
            panic!(
                "Data points in {} do not align with {}.",
...
impl AlignedChartDataSet {
    ...
    pub fn combine(&self, coefficients: &[f32]) -> Vec<f32> {
        let coeff_map: Vec<(usize, f32)> = coefficients
            .iter()
            .map(|x| *x)
            .enumerate()
            .filter(|x| x.1 != 0.0)
